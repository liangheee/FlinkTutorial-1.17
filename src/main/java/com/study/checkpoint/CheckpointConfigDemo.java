...
public class CheckpointConfigDemo {
    public static void main(String[] args) throws Exception {
        ...
        // TODO 开启最终检查点：flink1.15开始，默认是true
        //  这样才能够保证程序关闭时的最后一次检查点不会丢失  (因为flink程序关闭时，可能还有检查点在执行，并未结束，我们也需要保存这次的检查点，才不会丢失状态)
//        configuration.set(ENABLE_CHECKPOINTS_AFTER_TASKS_FINISH,false); // 把最终检查点关闭
        ...
        //  要求checkpoint的最大并发必须为1，其它参数建议在flink-conf配置文件中配置
...
//        checkpointConfig.setExternalizedCheckpointCleanup(CheckpointConfig.ExternalizedCheckpointCleanup.DELETE_ON_CANCELLATION);
        ...
        // 设置重启策略
        // 10分钟内如果失败次数等于10次（每次失败重启的时间间隔为10s），那么在第10次失败时，就会放弃该Flink任务
        env.setRestartStrategy(RestartStrategies.failureRateRestart(10, Time.minutes(10),Time.seconds(10)));
